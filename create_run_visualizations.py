#!/usr/bin/env python3
"""
Generate per-run visualizations from a single *_full.json results file.

Outputs (default: figures/):
- <prefix>_mean_ranks.png        : Mean rank of gold docs (A, B) per method
- <prefix>_rank_box.png          : Rank distribution boxplots for gold docs
- <prefix>_timings.png           : Per-query runtime distribution per method
- <prefix>_precision_recall.png  : P@3/P@5 and R@3/R@5 per method
"""
...
def precision_recall_at_k(attributions: List[float], doc_ids: List[str], gold: List[str], k: int) -> Tuple[float, float]:
    ...
    # Use absolute scores like the evaluation code
...
def plot_rank_box(df: pd.DataFrame, output_path: Path, method_order: List[str]):
    melted = df.melt(id_vars=["method", "query_idx", "top2"], value_vars=["rank_A", "rank_B"], var_name="doc", value_name="rank")
    plt.figure(figsize=(8, 5))
...
def plot_timings(df_timing: pd.DataFrame, output_path: Path, method_order: List[str]):
    ...
    sns.boxplot(data=df_timing, x="method", y="timing_seconds", hue="method", order=method_order, palette="pastel", legend=False)
...
def main():
    parser = argparse.ArgumentParser(description="Create visualizations for a single *_full.json run.")
    ...
    run = load_full_run(input_path)
    df, df_pr, df_timing, method_order = build_frames(run)

    prefix = input_path.stem.replace("_full", "")
